...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', type=str, default='PaviaU',#Salinas
                        help='DATASET NAME')
    ...
    parser.add_argument('--spc', type=int, default=100,
    ...
    for r in range(arg.run):
        ...
        gcn1.load_state_dict(torch.load(f"models/{arg.name}/{arg.spc}/{arg.block}_overall_skip_2_SGAT_l1_clip/intNet_best_{arg.spc}_{r}.pkl"))
        gcn2.load_state_dict(torch.load(f"models/{arg.name}/{arg.spc}/{arg.block}_overall_skip_2_SGAT_l1_clip/extNet_best_{arg.spc}_{r}.pkl"))
                # torch.load(f"models/{arg.name}/{arg.spc}/{arg.block}_overall_skip_2_SGConv_l1_clip/extNet_best_{arg.spc}_{r}.pkl"))
        ...
        seg_torch = torch.from_numpy(seg).to(device)

        # Create a map with default background value 0
        map = torch.zeros(seg_torch.shape, dtype=torch.long,device=device)
        # Fill the map with predictions for non-background pixels
        map[seg_torch != 0] = preds[seg_torch[seg_torch != 0]]
